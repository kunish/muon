...
function removeStatus(index: number) {
  ...
  workflow.value.transitions = workflow.value.transitions.filter(tr =>
    tr.from !== status.key && tr.to !== status.key,
  )
}
...
function updateStatusKey(status: WorkflowStatus, value: string | number) {
  ...
  for (const transition of workflow.value.transitions) {
    if (transition.from === oldKey)
      transition.from = nextKey
    if (transition.to === oldKey)
      transition.to = nextKey
  }
}

function updateStatusCategory(status: WorkflowStatus, value: string) {
  if (statusCategories.includes(value as StatusCategory))
    status.category = value as StatusCategory
}
...
<template>
  <div v-if="workflow" class="space-y-6 p-6">
    <div>
      ...
      <div class="space-y-2">
        <div v-for="(status, i) in workflow.statuses" :key="status.key" class="grid gap-2 rounded-lg border border-border p-3 sm:grid-cols-[8rem_minmax(0,1fr)_9rem_3.5rem_auto]">
          <Input
          ...
          <Input
            ...
            type="color"
            class="p-1"
            :aria-label="t('projects.status_color')"
          />
    ...
    <div>
      ...
      <div class="space-y-2">
        <div v-for="(tr, i) in workflow.transitions" :key="i" class="grid gap-2 rounded-lg border border-border p-3 sm:grid-cols-[minmax(0,1fr)_auto_minmax(0,1fr)_minmax(8rem,1fr)_auto]">
          <select v-model="tr.from" data-testid="project-workflow-transition-from" class="rounded border bg-background px-2 py-1.5 text-sm">
            <option v-for="s in workflow.statuses" :key="s.key" :value="s.key">
              {{ s.name }}
            </option>
          </select>
          <span class="text-muted-foreground">→</span>
          <select v-model="tr.to" data-testid="project-workflow-transition-to" class="rounded border bg-background px-2 py-1.5 text-sm">
            <option v-for="s in workflow.statuses" :key="s.key" :value="s.key">
              {{ s.name }}
            </option>
          </select>
          <Input v-model="tr.name" :placeholder="t('projects.transition_name')" />
          <Button variant="ghost" size="icon" data-testid="project-workflow-remove-transition" @click="removeTransition(i)">
            <Trash2 class="h-4 w-4 text-destructive" />
          </Button>
        </div>
      </div>
