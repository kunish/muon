...
import { AppWindow, BriefcaseBusiness, Brush, CalendarClock, Code2, Grid3X3, MessageSquare, MoreHorizontal, Plus, Search, Table2, UsersRound, Video } from 'lucide-vue-next'
...
const apps = shallowRef<WorkplaceAppEntry[]>([
  { id: 'calendar', name: '日历', desc: '团队日程、专注时间和会议安排', category: 'productivity', icon: CalendarClock, accent: 'text-warning', moduleLabel: '日历', path: '/calendar', actionLabel: '打开日历' },
  { id: 'meet', name: '视频会议', desc: '一键发起加密团队通话', category: 'productivity', icon: Video, accent: 'text-primary', moduleLabel: '通话', path: '/calls', actionLabel: '发起会议' },
  { id: 'base', name: '多维表格', desc: '结构化追踪业务风险、负责人和状态', category: 'operations', icon: Table2, accent: 'text-success', moduleLabel: '工作台', path: '/workplace', actionLabel: '打开表格' },
  { id: 'project', name: '项目管理', desc: '跟进跨端项目里程碑、风险和负责人', category: 'productivity', icon: BriefcaseBusiness, accent: 'text-primary', moduleLabel: '工作台', path: '/workplace', actionLabel: '查看项目' },
  { id: 'okr', name: 'OKR', desc: '团队目标、关键结果和信心度 check-in', category: 'operations', icon: AppWindow, accent: 'text-warning', moduleLabel: '工作台', path: '/workplace', actionLabel: '查看 OKR' },
  { id: 'tasks', name: '任务中心', desc: '个人待办与团队执行看板', category: 'operations', icon: AppWindow, accent: 'text-secondary', moduleLabel: '审批', path: '/approvals', actionLabel: '查看待办' },
  { id: 'standup', name: '站会机器人', desc: '工程团队异步同步进展', category: 'engineering', icon: MessageSquare, accent: 'text-success', moduleLabel: '消息', path: '/dm', actionLabel: '进入消息' },
])
...
const filteredBaseRecords = computed(() => {
  if (baseFilter.value === 'risks')
    return baseRecords.value.filter(record => record.status === '风险项')
  return baseRecords.value
})
const filteredProjectRecords = computed(() => {
  if (projectFilter.value === 'risks')
...
function moveAppDown(appId: string): void {
  ...
  if (currentIndex < 0 || currentIndex >= apps.value.length - 1)
    return
...
function hideAppEntry(appId: string): void {
  ...
  if (selectedAppId.value === appId)
    selectedAppId.value = filteredApps.value[0]?.id ?? apps.value[0]?.id ?? ''
        ...
        <div class="mx-auto grid w-full max-w-[1180px] gap-5 xl:grid-cols-[minmax(0,1fr)_340px]">
          <section class="flex min-w-0 flex-col gap-5">
            ...
            <div>
              ...
              <div v-if="appEditorOpen" class="mb-3 grid gap-2 rounded-lg border border-border bg-background p-3 md:grid-cols-[minmax(0,1fr)_minmax(0,1fr)_auto]">
                <input
          ...
          <aside class="workspace-surface h-fit overflow-hidden rounded-lg">
            ...
            <div class="border-b border-border p-4">
              <div class="flex items-start gap-3">
                <span class="flex size-10 shrink-0 items-center justify-center rounded-lg border border-border bg-muted" :class="selectedApp.accent">
                  <component :is="selectedApp.icon" :size="20" />
                </span>
                <span class="min-w-0 flex-1">
                  <span class="block text-[13px] font-semibold">{{ selectedApp.name }}</span>
                  <span class="mt-1 block text-[12px] leading-[18px] text-muted-foreground">{{ selectedApp.desc }}</span>
                </span>
              </div>
              <button
              ...
              <div v-if="selectedApp.id === 'base'" class="mt-4 rounded-lg border border-border">
                ...
                <div class="divide-y divide-border">
                  <div
                    v-for="record in filteredBaseRecords"
                    :key="record.id"
              ...
              <div v-if="selectedApp.id === 'project'" class="mt-4 rounded-lg border border-border">
                ...
                <div class="divide-y divide-border">
                  ...
                  >
                    <span class="font-semibold text-foreground">{{ record.title }}</span>
                    <span class="text-muted-foreground">{{ record.owner }} · {{ record.stage }} · {{ record.status }}</span>
                  </div>
                </div>
              </div>
