...
const selectedAssignee = computed<AssigneeOption | null>(() => {
  if (!props.modelValue)
    return null
  return optionById.value.get(props.modelValue) ?? {
...
const visibleOptions = computed<AssigneeOption[]>(() => {
  ...
  for (const option of contactOptions.value) {
    if (!term || matchesOption(option, term))
      options.set(option.userId, option)
  }

  if (term) {
    if (directOption.value)
      options.set(directOption.value.userId, directOption.value)
...
<template>
  <div class="space-y-2">
    ...
    >
      <div
        v-if="searching && visibleOptions.length === 0"
