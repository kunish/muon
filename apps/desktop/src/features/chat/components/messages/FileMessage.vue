...
<template>
  <div
    class="relative min-w-[260px] max-w-[360px] overflow-hidden rounded-lg border border-border bg-background transition-shadow duration-200 hover:shadow-[0_2px_8px_rgba(0,0,0,0.06)]"
  >
    <!-- File info row -->
    <div class="flex items-start gap-3 p-3">
      <div class="w-10 h-10 rounded-lg flex items-center justify-center shrink-0" :class="fileIconColor">
        <FileText :size="20" />
      </div>
    ...
    <div class="flex items-center border-t border-border/40 divide-x divide-border/40">
      ...
      <button
        data-testid="file-forward-button"
        class="flex flex-1 cursor-pointer items-center justify-center gap-1 py-[7px] text-xs text-muted-foreground transition-all duration-150 hover:bg-[color-mix(in_srgb,var(--color-primary)_5%,transparent)] hover:text-primary"
        @click.stop="forward"
      >
        <Forward :size="14" />
    ...
    <ForwardDialog v-if="showForwardDialog" :event="event" @close="showForwardDialog = false" />
  </div>
</template>
