...
const filtered = computed(() => {
  if (!query.value) return contactList.contacts;
  const q = query.value.toLowerCase();
  return contactList.contacts.filter(
    (c) => c.displayName.toLowerCase().includes(q) || c.userId.toLowerCase().includes(q),
  );
});
...
<template>
  <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/40" @click.self="emit('close')">
    <div
      class="bg-background border border-border rounded-xl shadow-xl w-[360px] max-h-[480px] flex flex-col overflow-hidden"
    >
      ...
      <div class="flex-1 overflow-y-auto px-2 pb-2">
        <div v-if="filtered.length === 0" class="text-center text-sm text-muted-foreground py-8">
        ...
        <button
          ...
          @click="onSelect(c)"
        >
          <Avatar :src="c.avatarUrl" :alt="c.displayName" :color-id="c.userId" size="sm" shape="circle" />
