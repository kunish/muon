...
watch(
  ...
  (items) => {
    for (const item of items) {
      if (!item.avatar || avatarCache.value[item.avatar]) continue;
...
<template>
  <div
    ref="listRef"
    class="bg-popover/95 backdrop-blur-xl border border-border/60 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] py-1.5 max-h-[240px] overflow-y-auto min-w-[200px]"
  >
    <button
