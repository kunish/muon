...
const rowStateClass = computed(() => {
  ...
  if (pinned.value) {
    return isContextMenuOpen.value ? 'conv-pinned bg-accent' : 'conv-pinned bg-muted hover:bg-accent';
  }

  return isContextMenuOpen.value ? 'bg-accent' : 'hover:bg-accent';
});
const avatarStateClass = computed(() => (isContextMenuOpen.value ? 'scale-[1.04]' : ''));
const nameTextClass = computed(() => {
  if (props.active || isUnreadOrMarked.value) return 'text-foreground';
  return isContextMenuOpen.value ? 'text-foreground/95' : 'text-foreground/80 group-hover:text-foreground/95';
});
const timeTextClass = computed(() => {
  if (props.active || props.room.unreadCount > 0) return 'text-primary font-semibold';
...
const previewTextClass = computed(() => {
  if (props.active || isUnreadOrMarked.value) return 'text-muted-foreground/90';
  return isContextMenuOpen.value
    ? 'text-muted-foreground/70'
...
const groupMemberCount = computed(() => {
  if (props.room.isDirect) return 0;
...
const preview = computed(() => {
  if (typeLabel.value) return `[${typeLabel.value}]`;
  if (!props.room.lastMessage) return t('chat.no_messages');
  return props.room.lastMessage;
});

const sender = computed(() => {
  if (props.room.isDirect || !props.room.lastMessageSender) return '';
...
<template>
  ...
  >
    <span v-if="active" class="absolute left-0 top-1/2 h-8 w-[3px] -translate-y-1/2 bg-primary" />
    ...
    <div class="flex-1 min-w-0">
      <div class="flex items-center justify-between gap-2">
        ...
        <div class="flex items-center gap-1 shrink-0">
          ...
          <span class="text-[10px] tabular-nums tracking-tight transition-colors duration-150" :class="timeTextClass">
            {{ timeLabel }}
          </span>
        </div>
      </div>
      ...
      <div class="mt-[3px] flex items-center justify-between gap-2">
        ...
        >
          ...
          <template v-else>
            <!-- 飞书风格：@提及标识 -->
            <span v-if="hasHighlight" class="shrink-0 text-destructive/80 font-semibold"
              >[{{ t('chat.at_mention') }}]</span
            >
            <component :is="typeIcon" v-if="typeIcon" :size="11" class="shrink-0 opacity-50" />
            <span v-if="sender" class="shrink-0 max-w-[56px] truncate text-foreground/35"> {{ sender }}: </span>
        ...
        <span v-else-if="room.unreadCount > 0 && muted" class="shrink-0 w-2 h-2 rounded-full bg-muted-foreground/30" />
        <span v-else-if="markedUnread" class="shrink-0 w-2.5 h-2.5 rounded-full bg-primary/60" />
      </div>
    </div>
  </div>
</template>
