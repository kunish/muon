...
const PREVIEW_BROWSER_USER_AGENT =
...
if (!_g.__ogCache) _g.__ogCache = new Map<string, OgCacheEntry>();
if (!_g.__ogInflight) _g.__ogInflight = new Map<string, Promise<OgCacheEntry | null>>();
...
function getFaviconUrl(doc: Document, baseUrl: string): string {
  const iconLink = Array.from(doc.querySelectorAll<HTMLLinkElement>('link[rel][href]')).find((link) => {
...
async function fetchPreviewResponse(
  ...
  for (let redirectCount = 0; redirectCount <= MAX_PREVIEW_REDIRECTS; redirectCount += 1) {
    ...
    if (!isRedirectResponse(resp)) return { resp, baseUrl: resp.url || currentUrl.href };

    const location = resp.headers.get('location');
    if (!location) return { resp, baseUrl: currentUrl.href };
...
function isAllowedImageResponse(resp: Response): boolean {
  ...
  if (contentLength !== null) {
    ...
    if (!Number.isFinite(bytes) || bytes > MAX_PREVIEW_IMAGE_BYTES) return false;
  }
...
async function fetchPreviewImageBlobUrl(rawUrl: string): Promise<string> {
  ...
  if (!imageUrl || typeof URL.createObjectURL !== 'function') return '';
  ...
  try {
    ...
    if (!resp || !isAllowedImageResponse(resp)) return '';

    const blob = await resp.blob();
    if (!blob.size || blob.size > MAX_PREVIEW_IMAGE_BYTES) return '';
...
function setFavicon(url: string) {
  if (faviconBlobUrl && faviconBlobUrl !== url) revokePreviewBlobUrl(faviconBlobUrl);
  faviconBlobUrl = url.startsWith('blob:') ? url : '';
  favicon.value = url;
}

function setOgImage(url: string) {
  if (ogImageBlobUrl && ogImageBlobUrl !== url) revokePreviewBlobUrl(ogImageBlobUrl);
...
async function hydrateAssetImages(entry: OgCacheEntry) {
  if (entry.favicon) {
    ...
    if (!unmounted && faviconUrl) setFavicon(faviconUrl);
  }

  if (entry.ogImage) {
    const imageUrl = await fetchPreviewImageBlobUrl(entry.ogImage);
    if (!unmounted && imageUrl) setOgImage(imageUrl);
  }
}

function createEntryFromDocument(doc: Document, baseUrl: string, domainVal: string): OgCacheEntry {
  const siteName = getMetaContent(doc, ['meta[property="og:site_name"]', 'meta[name="application-name"]']);
...
async function fetchOgData(url: string, domainVal: string): Promise<OgCacheEntry | null> {
  ...
  try {
    for (const headers of PREVIEW_HTML_HEADER_VARIANTS) {
      ...
      if (!resp || !isHtmlPreviewResponse(resp)) continue;
      ...
      if (hasUsefulPreviewMetadata(entry, domainVal)) return entry;
    }
...
onMounted(async () => {
  ...
  if (cached) OG_CACHE.delete(props.url);
  ...
  if (entry) {
    ...
    if (!entry.isFallback && OG_CACHE.size >= OG_CACHE_MAX) {
      ...
      if (firstKey !== undefined) OG_CACHE.delete(firstKey);
    }
    if (!entry.isFallback) OG_CACHE.set(props.url, entry);
    applyResult(entry);
  } else {
    // 安全策略拦截的 URL 不显示预览卡片。
    failed.value = true;
    ...
    <div class="flex items-start gap-3.5 px-4 pb-3 pt-1">
      <div class="flex-1 min-w-0">
        ...
        <div class="mb-1.5 flex h-5 items-center gap-1.5">
          ...
          <template v-else>
            <img v-if="favicon" :src="favicon" alt="" class="w-4 h-4 rounded-sm shrink-0" @error="setFavicon('')" />
        ...
        <div data-testid="link-preview-description-slot" class="mt-1.5 min-h-[38px]">
          <template v-if="loading">
      ...
      >
        <div v-if="loading" class="h-full w-full bg-muted/50 animate-pulse" />
        <img v-else-if="ogImage" :src="ogImage" alt="" class="h-full w-full object-contain" @error="setOgImage('')" />
