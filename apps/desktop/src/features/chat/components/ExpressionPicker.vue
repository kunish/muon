...
<template>
  <div
    class="flex h-[min(460px,calc(100vh-24px))] min-h-0 w-[min(360px,calc(100vw-16px))] flex-col overflow-hidden rounded-xl border border-border bg-popover shadow-2xl"
  >
    ...
    <div
      class="min-h-0 flex-1 [&_.emoji-picker]:size-full [&_.emoji-picker]:rounded-none [&_.emoji-picker]:border-0 [&_.emoji-picker]:bg-transparent [&_.emoji-picker]:shadow-none [&_.gif-picker]:size-full [&_.gif-picker]:rounded-none [&_.gif-picker]:border-0 [&_.gif-picker]:bg-transparent [&_.gif-picker]:shadow-none [&_.sticker-picker]:size-full [&_.sticker-picker]:rounded-none [&_.sticker-picker]:border-0 [&_.sticker-picker]:bg-transparent [&_.sticker-picker]:shadow-none"
    >
      <EmojiPicker v-if="mountedTabs.emoji" v-show="activeTab === 'emoji'" @select="emit('selectEmoji', $event)" />

      <GifPicker v-if="mountedTabs.gif" v-show="activeTab === 'gif'" @select="emit('selectGif', $event)" />
