...
<template>
  <div
    class="flex items-center py-0.5 px-4 pl-14 min-h-[1.375rem] hover:bg-background/[0.06] group animate-sys-fade-in"
  >
    <!-- 图标列 -->
    <component :is="icon" :size="14" class="shrink-0 text-muted-foreground/40 mr-2" />
    ...
    <span class="text-xs text-muted-foreground/60 leading-snug">
      <template v-for="(part, idx) in info.parts" :key="idx">
        ...
        >
        <!-- 高亮文本（群名/话题等） -->
        <span v-else-if="part.type === 'highlight'" class="font-medium text-foreground/70">{{ part.text }}</span>
        <!-- 普通文本 -->
    ...
    <!-- 时间戳 -->
    <span
      class="ml-2 text-[11px] text-muted-foreground/30 opacity-0 group-hover:opacity-100 transition-opacity shrink-0"
    >
      {{ timestamp }}
    </span>
  </div>
