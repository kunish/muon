...
const currentValue = computed(() => {
  if (!retention.value?.enabled) return null;
...
<template>
  <div
    class="absolute right-0 top-full mt-1 w-52 bg-popover/95 backdrop-blur-xl border border-border/60 rounded-xl shadow-[0_8px_30px_rgba(0,0,0,0.12)] py-1.5 z-30"
  >
    ...
    >
      <span>{{ opt.label }}</span>
      <span v-if="opt.value === currentValue" class="w-1.5 h-1.5 rounded-full bg-primary" />
    </button>
  </div>
</template>
