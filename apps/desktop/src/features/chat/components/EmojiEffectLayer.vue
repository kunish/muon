...
function getPreset(emoji: string): EffectPreset {
  if (PRESETS[emoji]) return PRESETS[emoji];
...
// ---- 生成单波粒子 ----
function spawnWave(preset: EffectPreset, sx: number, sy: number, count: number) {
...
function tick(time = 0) {
  ...
  for (const p of particles) {
    ...
    if (p.life > 0.01) {
      ...
      ctx.drawImage(tex, (-drawSize * dpr) / 2, (-drawSize * dpr) / 2, drawSize * dpr, drawSize * dpr);
      ctx.restore();
...
function trigger(emoji: string, sourceRect: DOMRect) {
  ...
  if (burst >= 2) totalCount = Math.max(18, Math.floor(totalCount * 0.7));
  if (burst >= 4) totalCount = Math.max(14, Math.floor(totalCount * 0.55));
  const perWave = Math.ceil(totalCount / preset.waves);

  for (let w = 0; w < preset.waves; w++) {
    const count = w === preset.waves - 1 ? totalCount - perWave * (preset.waves - 1) : perWave;
...
<template>
  <canvas ref="canvasRef" class="pointer-events-none absolute inset-0 z-50 contain-strict" />
</template>
