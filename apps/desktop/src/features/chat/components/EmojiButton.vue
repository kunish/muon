...
function onClickOutside(e: MouseEvent) {
  ...
  if (btnRef.value?.contains(target) || pickerRef.value?.contains(target)) return;
  open.value = false;
}
...
<template>
  <div ref="btnRef">
    <button class="p-1.5 rounded-md hover:bg-accent text-muted-foreground" @click="toggle">
      <Smile :size="18" />
    </button>
    <Teleport to="body">
      <div v-if="open" ref="pickerRef" class="fixed z-50" :style="{ left: pickerStyle.left, top: pickerStyle.top }">
