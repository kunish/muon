...
const homeHighlight = computed(() =>
  conversations.value.filter((r) => r.isDirect).reduce((sum, r) => sum + r.highlightCount, 0),
);

function homeIndicatorClass(): string {
  if (isDmMode.value) return 'w-5 opacity-100';
  if (homeHighlight.value > 0) return 'w-2.5 opacity-100';
  if (homeUnread.value > 0) return 'w-1.5 opacity-100';
...
<template>
  <TooltipProvider>
    <nav
      class="muon-scrollbar-hidden flex shrink-0 flex-col items-center gap-1.5 overflow-y-auto bg-server-bar py-2 w-[52px]"
    >
      <!-- Home / DM Button -->
      <Tooltip>
      ...
      <!-- Server Icons -->
      <Tooltip v-for="(server, index) in servers" :key="server.spaceId">
        <TooltipTrigger as-child>
          <div
            class="group relative flex cursor-pointer flex-col items-center justify-center gap-[3px]"
          ...
          <Tooltip>
            <TooltipTrigger as-child>
              <button class="group relative flex cursor-pointer flex-col items-center justify-center gap-[3px]">
                <div
                  class="flex size-9 items-center justify-center overflow-hidden rounded-xl bg-background text-[15px] font-semibold text-muted-foreground transition-all duration-150 hover:bg-primary/10 hover:text-primary"
                >
                  <Plus :size="20" />
                </div>
              </button>
