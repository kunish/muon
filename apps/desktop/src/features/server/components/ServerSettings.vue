...
<template>
  <Teleport to="body">
    ...
    >
      <div v-if="open && currentServer" class="fixed inset-0 z-[100] flex bg-popover">
        <!-- Left nav sidebar -->
        ...
        <div class="relative flex flex-1 flex-col overflow-hidden">
          ...
          <div class="absolute right-[9px] top-[96px] text-[11px] font-semibold text-muted-foreground/50">ESC</div>
          ...
          <div class="muon-scrollbar muon-scrollbar-compact flex-1 overflow-y-auto py-[60px] pl-10 pr-20">
            <div class="max-w-[660px]">
              ...
              />
              <RoleManager v-else-if="currentSection === 'roles'" :server-id="currentServer.spaceId" />
              <ChannelManager v-else-if="currentSection === 'channels'" :server-id="currentServer.spaceId" />
              <MemberManager v-else-if="currentSection === 'members'" :server-id="currentServer.spaceId" />
            </div>
          </div>
        </div>
      </div>
