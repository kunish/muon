...
const isActive = computed(() => currentChannelId.value === props.channel.roomId && isConnected.value);

const isUnread = computed(() => props.channel.unreadCount > 0);
const hasMentions = computed(() => props.channel.highlightCount > 0);

const isJoining = computed(() => currentChannelId.value === props.channel.roomId && isConnecting.value);

const usersInChannel = computed(() => {
  if (isActive.value) return voiceUsers.value;
  return props.connectedUsers ?? [];
});

const memberCount = computed(() => usersInChannel.value.length);
const rowStateClass = computed(() => {
  if (isActive.value) return 'bg-accent text-foreground font-medium';
  if (props.contextMenuOpen === true) return 'bg-accent/30 text-foreground';
...
<template>
  <div class="relative">
    ...
    <button
      ...
      :class="[rowStateClass, isUnread && !isActive ? 'font-semibold text-foreground' : '']"
      @click="onJoinVoice"
    >
      <Volume2 :size="20" class="shrink-0 opacity-70" :class="isActive ? 'text-foreground' : 'text-muted-foreground'" />
      <span class="truncate">{{ channel.name }}</span>
      <span v-if="isJoining" class="ml-auto text-[11px] text-muted-foreground">{{ t('voice.connecting') }}</span>
      <span
        v-else-if="hasMentions"
        class="ml-auto inline-flex min-w-[18px] h-[18px] items-center justify-center rounded-full bg-destructive px-1 text-[11px] font-bold text-destructive-foreground"
      >
    ...
    <div v-if="usersInChannel.length" class="ml-7 space-y-0.5 pb-1">
      ...
      >
        <Avatar :src="user.avatarUrl" :alt="user.displayName" :color-id="user.userId" size="sm" class="!h-6 !w-6" />
