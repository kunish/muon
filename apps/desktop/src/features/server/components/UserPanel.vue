...
const { isMuted, isDeafened, toggleMute, toggleDeafen } = useVoiceChannel();

const actionButtonClass =
  'flex h-8 w-8 cursor-pointer items-center justify-center rounded-xl text-muted-foreground transition-all duration-150 hover:bg-sidebar-accent hover:text-foreground active:scale-95';
...
<template>
  <div
    class="user-panel flex shrink-0 items-center gap-2 border-t border-sidebar-border/80 bg-sidebar/90 px-3 py-2.5 backdrop-blur-xl"
  >
    <div class="flex min-w-0 flex-1 items-center gap-2">
      <Avatar :src="currentUser.mxcAvatar" :alt="currentUser.displayName" :color-id="currentUser.userId" size="sm" />
    ...
    <div class="flex shrink-0 items-center gap-0.5">
      ...
      <button :class="actionButtonClass" :title="t('settings.settings')" @click="openSettings">
        <Settings :size="16" />
      </button>
    </div>
  </div>
