...
describe('local service mock data', () => {
  ...
  it('references only seeded local users', () => {
    ...
    for (const dm of LOCAL_SERVICE_MOCK_DATA.dmRooms) {
      ...
      for (const message of dm.messages) expectKnownUser(message.sender)
    }

    for (const room of LOCAL_SERVICE_MOCK_DATA.groupRooms) {
      for (const member of room.members) expectKnownUser(member)
      for (const message of room.messages) expectKnownUser(message.sender)
    }

    for (const space of LOCAL_SERVICE_MOCK_DATA.spaces) {
      for (const member of space.members) expectKnownUser(member)

      for (const channel of space.channels) {
        for (const member of channel.members) expectKnownUser(member)
        for (const message of channel.messages) expectKnownUser(message.sender)
      }
  ...
  it('uses stable unique keys for seed markers', () => {
    const keys = [
      ...
      ...LOCAL_SERVICE_MOCK_DATA.spaces.flatMap((space) => [
        ...
        ...space.categories.flatMap((category) => [category.key, ...category.channels.map((channel) => channel.key)]),
      ]),
    ]
