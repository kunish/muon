...
async function installAndLogin(handler: ReturnType<typeof createEnterpriseHttpHandler>) {
  ...
  const login = await handler.fetch(
    new Request('http://muon.test/api/admin/login', {
      method: 'POST',
      headers: { 'content-type': 'application/json' },
      body: JSON.stringify({
        organizationSlug: 'acme',
...
describe('enterprise api routes', () => {
  ...
  it('reports install status before and after install', async () => {
    ...
    const install = await handler.fetch(
      new Request('http://muon.test/api/install', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  it('lets authenticated admins create and list multiple organizations', async () => {
    ...
    const create = await handler.fetch(
      new Request('http://muon.test/api/admin/organizations', {
        method: 'POST',
        headers: {
  ...
  it('lets authenticated admins update owner details and reset owner passwords', async () => {
    ...
    const installResponse = await handler.fetch(
      new Request('http://muon.test/api/install', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
    ...
    const update = await handler.fetch(
      new Request(`http://muon.test/api/admin/users/${installPayload.owner.id}`, {
        method: 'PATCH',
        headers: {
          authorization: `Bearer ${loginPayload.session.accessToken}`,
          'content-type': 'application/json',
    ...
    const reset = await handler.fetch(
      new Request(`http://muon.test/api/admin/users/${installPayload.owner.id}/password`, {
        method: 'POST',
        headers: {
          authorization: `Bearer ${loginPayload.session.accessToken}`,
          'content-type': 'application/json',
    ...
    const relogin = await handler.fetch(
      new Request('http://muon.test/api/admin/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  async function setupMustChangeOwner() {
    ...
    return {
      ...
      ownerId: installPayload.owner.id,
    }
  }
  ...
  it('lets a must-change user still hit /api/admin/me', async () => {
    const { handler, token } = await setupMustChangeOwner()
    const response = await handler.fetch(
      new Request('http://muon.test/api/admin/me', {
        headers: { authorization: `Bearer ${token}` },
      }),
    )
  ...
  it('keeps admin sessions valid across handler recreation when sharing a repository', async () => {
    ...
    const login = await handlerA.fetch(
      new Request('http://muon.test/api/admin/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  it('logs out an admin session, then rejects further requests with that token', async () => {
    ...
    const logout = await handler.fetch(
      new Request('http://muon.test/api/admin/logout', {
        method: 'POST',
        headers: { authorization: `Bearer ${token}` },
      }),
    )
  ...
  it('lets a must-change user change their own password, clears the flag, and unlocks gated endpoints', async () => {
    ...
    const change = await handler.fetch(
      new Request('http://muon.test/api/admin/me/password', {
        method: 'POST',
        headers: {
    ...
    const orgs = await handler.fetch(
      new Request('http://muon.test/api/admin/organizations', {
        headers: { authorization: `Bearer ${token}` },
      }),
    )
  ...
  it("revokes the user's other admin sessions on successful password change", async () => {
    ...
    const secondLogin = await handler.fetch(
      new Request('http://muon.test/api/admin/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  it('pOST /api/oauth/refresh issues a new session pair', async () => {
    ...
    const loginRes = await handler.fetch(
      new Request('http://muon.test/api/oauth/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          organizationSlug: 'acme',
    ...
    const exchangeRes = await handler.fetch(
      new Request('http://muon.test/api/oauth/token', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
          code,
    ...
    const refreshRes = await handler.fetch(
      new Request('http://muon.test/api/oauth/refresh', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
  ...
  async function setupAdminWithDeviceSession() {
    ...
    // Admin login.
    const adminLogin = await handler.fetch(
      new Request('http://muon.test/api/admin/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
    ...
    const meRes = await handler.fetch(
      new Request('http://muon.test/api/admin/me', {
  ...
  it('gET /api/admin/users/:userId/sessions returns active desktop sessions without hashes', async () => {
    ...
    const response = await handler.fetch(
      new Request(`http://muon.test/api/admin/users/${ownerId}/sessions`, {
        headers: { authorization: `Bearer ${adminToken}` },
      }),
    )
    expect(response.status).toBe(200)
  ...
  it('gET /api/admin/users/:userId/sessions returns 403 for a must-change-password admin', async () => {
    ...
    const response = await handler.fetch(
      new Request('http://muon.test/api/admin/users/some-user/sessions', {
  ...
  it('dELETE /api/admin/users/:userId/sessions/:sessionId revokes the session', async () => {
    ...
    const del = await handler.fetch(
      new Request(`http://muon.test/api/admin/users/${ownerId}/sessions/${sessionId}`, {
        method: 'DELETE',
        headers: { authorization: `Bearer ${adminToken}` },
      }),
    )
  ...
  it('dELETE /api/admin/users/:userId/sessions/:sessionId returns 404 for an unknown session id', async () => {
    const { handler, adminToken, ownerId } = await setupAdminWithDeviceSession()
    const del = await handler.fetch(
      new Request(`http://muon.test/api/admin/users/${ownerId}/sessions/not-a-session`, {
        method: 'DELETE',
        headers: { authorization: `Bearer ${adminToken}` },
      }),
    )
  ...
  it('dELETE /api/admin/users/:userId/sessions/:sessionId returns 404 when session belongs to a different org (cross-org bypass prevention)', async () => {
    ...
    const adminLoginA = await handlerA.fetch(
      new Request('http://muon.test/api/admin/login', {
        method: 'POST',
        headers: { 'content-type': 'application/json' },
        body: JSON.stringify({
    ...
    const del = await handlerA.fetch(
      new Request(`http://muon.test/api/admin/users/${userB.id}/sessions/${sessionB.id}`, {
        method: 'DELETE',
