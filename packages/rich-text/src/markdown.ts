...
const BLOCK_TAGS = new Set(['blockquote', 'div', 'h1', 'h2', 'h3', 'h4', 'h5', 'h6', 'li', 'ol', 'p', 'pre', 'ul'])
...
export function renderMarkdownForMatrix(source: string): MatrixMarkdownRender | null {
  ...
  if (!markdown || !looksLikeMarkdown(markdown)) return null
  ...
  if (!formattedBody || !body) return null
...
function looksLikeMarkdown(source: string): boolean {
  return INLINE_MARKDOWN_RE.test(source) || BLOCK_MARKDOWN_RE.test(source) || HTML_MARKDOWN_RE.test(source)
}
...
function collectText(node: Node): string {
  ...
  if (node instanceof HTMLElement && BLOCK_TAGS.has(node.tagName.toLowerCase())) return `${text}\n`
