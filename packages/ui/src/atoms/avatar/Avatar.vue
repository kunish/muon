...
// mxc 解析
const resolvedMxc = useAuthMedia(() => (isMxc.value ? props.src : undefined), thumbnailPx.value, thumbnailPx.value);

// 最终 src：mxc 走 resolvedMxc，其他直接用
const finalSrc = computed(() => {
  if (imgError.value) return undefined;
  if (isMxc.value) return resolvedMxc.value;
  return props.src;
});
...
const fallbackGradient = computed(() => {
  if (props.colorId) return generateGradient(props.colorId);
  if (props.alt) return generateGradient(props.alt);
  return '';
});
...
const presenceDotSize = computed(() => {
  switch (props.size) {
    ...
    case 'sm':
      return 'w-2 h-2 ring-[1.5px]';
    case 'md':
      return 'w-2.5 h-2.5 ring-2';
    case 'lg':
      return 'w-3 h-3 ring-2';
    case 'xl':
      return 'w-3.5 h-3.5 ring-[3px]';
    case '2xl':
      return 'w-4 h-4 ring-[3px]';
    default:
      return 'w-2.5 h-2.5 ring-2';
  }
});
...
const avatarClasses = computed(() =>
  cn('relative flex h-full w-full items-center justify-center overflow-hidden', SHAPE_CLASSES[props.shape]),
);
</script>
